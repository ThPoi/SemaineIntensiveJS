...
function end(){
  ...
  let deadText = "Vous avez tué " + compteurXp + " de mort"
  let xpText = "Vous avez " + compteurDead + "xp"

  let dead = document.createElement("p")
  let xp = document.createElement("p")

  dead.style.zIndex = 21
  xp.style.zIndex = 21
  ...
  document.getElementById("textEnd").appendChild(dead)
  document.getElementById("textEnd").appendChild(xp)

  }
  // variable for good script
  ...
  function portraitCharacter(id) {
    if (id === 2) {
      ...
      portraitMonster.setAttribute("src", "images/barre_haut/portrait_naga.png")
    }else {
  ...
  function managementHealth () {
    ...
    setTimeout(function(){
      if (monster.health > 0 && player.health > 0) {
        ...
        // display message dead in textwrite
        ...
        // let time for animation dead
        animationMonster = 1
        turnMonster = 1
        generatorMonster ()
        stopDeadLoop = 0
      } else {
        console.log("Error management health")
      }
    }, 1000);
